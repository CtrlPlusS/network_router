...
struct MAC_ADDRESS mac_lan = {0x2c, 0xcf, 0x67, 0x2e, 0x1f, 0x85};
struct MAC_ADDRESS mac_wan = {0xb0, 0x38, 0x6c, 0xf1, 0x28, 0x4b};
...
void get_wan_ip(int sock){
    ...
    strncpy(ifr.ifr_name, "eth1", IFNAMSIZ-1); // wan interface
...
void get_lan_ip(int sock){
    ...
    ifr.ifr_addr.sa_family = AF_INET;
    strncpy(ifr.ifr_name, "wlan0", IFNAMSIZ-1); // lan interface
    if(ioctl(sock, SIOCGIFADDR, &ifr) < 0){
